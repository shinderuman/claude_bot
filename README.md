...
- **Go**: プログラミング言語（ビルドに必要）
- **rclone**: ファイル転送ツール（デプロイに必須）

#### rcloneのインストール

デプロイスクリプト実行にはrcloneが必須です：
...
- **ファイル同期**: rcloneを使用した高速・安全なファイル転送
- **双方向同期**: sessions.jsonを新旧比較で双方向同期
- **Supervisor管理**: デプロイ前に停止し、後に自動で開始
- **エラーハンドリング**: ビルド失敗やファイル存在チェック
- **進捗表示**: 番号付きステップで実行状況を可視化
- **設定の柔軟性**: SFTP接続情報を設定変数で管理
...
- **.envファイル**: ローカルからリモートへ一方向同期
- **sessions.json**: 双方向同期（新しい方のファイルを優先）
- **rclone使用**: 高速で安全なファイル転送と同期管理
- **自動検出**: ファイルの存在を自動チェックしてエラーを防止

### 実行フロー

1. **環境チェック**: rcloneのインストールを確認
